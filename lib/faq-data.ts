...
// from the JSON-LD.
//
// `category` drives the FAQExplorer's topical pills on /faq. Three buckets:
//   repair    — what we do to the car (timeline, parts, paint, ADAS, warranty, scope)
//   money     — insurance, rental, total-loss settlement
//   logistics — getting the car here, where it lives, updates, delivery
// Tag every entry. The cluster pill order in the UI matches the order of the
// FAQCategory union below.
...
export const FAQ_CLUSTERS: ReadonlyArray<{
  id: FAQCategory;
  label: string;
}> = [
